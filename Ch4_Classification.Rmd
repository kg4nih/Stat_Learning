...
```{r}
# log model on training set using just displacement as the predictor of mpg01
log_fit <- Auto_trn %>% 
           train(mpg01 ~ displacement, method = "glm", family = binomial, data = .)
summary(log_fit)
```

```{r}
log_fit2 <- Auto_trn %>% 
            train(mpg01 ~ displacement + weight, method = "glm", family = binomial, data = .)
summary(log_fit2)
```
