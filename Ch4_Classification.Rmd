...
```{r}
# mpg01 is a binary indicator; need to make it a factor or the prediction models will treat it as numeric 
Auto <- as.tibble(Auto)
Auto <- Auto %>% 
...
##### 11b - visuals
```{r}
# excluding name and mpg01
pairs(Auto[,1:8])
```
```{r}
# some boxplots of mpg01 (note mpg01 is a feature w/ 2 classes) and predictors
Auto %>% 
  ggplot(aes(mpg01, mpg)) +
  geom_boxplot(aes(group = mpg01))
```
```{r}
Auto %>% 
...
```{r}
Auto %>% 
  ggplot(aes(mpg01, origin)) +
  geom_count()
```
```{r}
Auto %>% 
  ggplot(aes(mpg01, year)) +
  geom_count()
```


##### 11c - Splitting into training and tests sets
```{r}
# using the caret package to split the data into training and test sets. The parameters in the call split the data 50/50 wile maintaining in class distribution for mpg01 
trnIndex <- createDataPartition(Auto$mpg01, p = .5, list = FALSE, times = 1)
Auto_trn <-  Auto[trnIndex,]
Auto_trn
Auto_tst <- Auto[-trnIndex,]
Auto_tst
cor(Auto_trn[,1:7])
```
...
```{r}
#using featureplot
featurePlot(x = Auto_trn[, c("mpg", "horsepower", "weight")], 
            y = Auto_trn$mpg01, 
            plot = "pairs",
...
##### 11f - LOGISTIC
```{r}
# log model on training set using just weight as the predictor of mpg01
log_fit <- Auto_trn %>% 
           train(mpg01 ~ weight, method = "glm", family = binomial, data = .)
summary(log_fit)
```

```{r}
log_fit2 <- Auto_trn %>% 
            train(mpg01 ~ weight + acceleration, method = "glm", family = binomial, data = .)
summary(log_fit2)
```
```{r}
l
...
```{r}
calc_class_err(predicted = log_pred, actual = Auto_tst$mpg01)
```
log is not particularly good. Only about 4% better than guessing 
```{r}
log_pred <- Auto_tst %>%  
            predict(log_fit2, data = ., type = "raw")
log_pred
log_tab <- table(predicted = log_pred, actual = Auto_tst$mpg01)
log_conf <- confusionMatrix(log_tab, positive = "1")
log_conf
```

##### 11d Linear Discriminant Analysis
```{r}
# data is already split into a training and test set. Going to start w/ weight and acceleration as predictors
lda_fit <- Auto_trn %>% 
           train(mpg01 ~ weight + acceleration, method = "lda", data = .)
lda_fit$finalModel # for LDA the summary is in $final$Model
```
```{r}
lda_pred <- predict(lda_fit, newdata = Auto_tst, type = "raw") # getting the classification results rather than the probs
lda_tab <- table(predicted = lda_pred, actual = Auto_tst$mpg01)
lda_conf <- confusionMatrix(lda_tab, positive = "1")
lda_conf
```
```{r}
calc_class_err(predicted = lda_pred, actual = Auto_tst$mpg01)
```
Wow - LDA is much better than logistic using the same predictors of weight and acceleration; LDA is correct almost 89% of the time. I tried adding horsepower but it didn't improve the accuracy .

##### 11e Quadratic Discriminant Analysis - QDA
```{r}
# QDA using weigth and acceleration as predictors of mpg01
qda_fit <- Auto_trn %>% 
            train(mpg01 ~ weight + acceleration, method = "qda", data = .)
qda_fit$finalModel
```
```{r}
qda_pred <- predict(qda_fit, newdata = Auto_tst, type = "raw") # getting the classification results rather than the probs
qda_tab <- table(predicted = qda_pred, actual = Auto_tst$mpg01)
qda_conf <- confusionMatrix(qda_tab, positive = "1")
qda_conf
```
```{r}
calc_class_err(predicted = qda_pred, actual = Auto_tst$mpg01)
```
QDA is a little worse than LDA using the same predictors

##### 11g - kNN
```{r}
# for kNN - need to scale weight and acceleration since they have measurement scales whic will effect distance mesurements in kNN
Auto_trn <- Auto_trn %>% 
            mutate_at(vars(weight, acceleration), funs(scale(.)))
Auto_tst <- Auto_tst %>% 
            mutate_at(vars(weight, acceleration), funs(scale(.)))
```
```{r}
# letting train find the best kNN - using the same predictors of mpg01 
ctlr <- trainControl(method = "cv", number = 10) # using cross-validtion w/ 10 folds
knn_fit <- Auto_trn %>% 
                    train(mpg01 ~ weight + acceleration, data = ., method = "knn", trControl = ctlr, preProcess = c("center", "scale"), tuneLength = 50, na.action = "na.omit", metric = "Kappa")
print(knn_fit)
```
```{r}
plot(knn_fit)
```
```{r}
knn_pred <- predict(knn_fit, newdata = Auto_tst, type = "raw")
knn_tab <- table(predicted = knn_pred, actual = Auto_tst$mpg01)
knn_conf <- confusionMatrix(knn_tab, positive = "1")
knn_conf
```
```{r}
calc_class_err(predicted = knn_pred, actual = Auto_tst$mpg01)
```
kNN w/ k = 5 is a little worse than LDA and QDA. Best classification model for mpg01 using weigth and acceleration is LDA w/ 89% accuracy

#### 12 Writing Functions
##### 12a
```{r}
Power  <- function() {
  print(2^3) # simple function
}
```
##### 12b and c
```{r}
...
##### 12e
```{r}
test <- tibble(x = 1:10, y = Power3(x,2))
test %>% 
  ggplot(aes(x,y)) +
    geom_line()
```
...
PlotPower(1:10, 4)
```
#### 13 Classification Models on Boston Data Set
##### 13a - Creating new binary class variable and exploring predictors
```{r}
...
```
```{r}
# checking correlation - looking for likely predictors that have high correlation w/ crime rate
cor(Boston[1:14])
# looks like rad would be a good starting point and chas since it's relatively un-correlated w/ rad
```
```{r}
# using the caret package to split the data into training and test sets. The parameters in the call split the data 50/50 wile maintaining in class distribution for mpg01 
trnIndex <- createDataPartition(Boston$crim01, p = .5, list = FALSE, times = 1)
Boston_trn <-  Boston[trnIndex,]
Boston_trn
Boston_tst <- Boston[-trnIndex,]
Boston_tst 
```

##### 13a - Logistic Model
```{r}
# fitting log model using all the predictors except crim since the log model will not converge w/ it as a predictor
log_fit <- Boston_trn %>% 
           train(crim01 ~ . - crim, method = "glm", family = binomial, data = .)
summary(log_fit)  
```
```{r}
# running prediction on the test data and checking results
log_pred <- knn_pred <- predict(log_fit, newdata = Boston_tst, type = "raw")
log_tab <- table(predicted = log_pred, actual = Boston_tst$crim01)
log_conf <- confusionMatrix(log_tab, positive = "1")
log_conf
```
Wonder if a simpler model could be almost as accurate
```{r}
# fitting log model using just the highest significant predictors (nox & rad) from the full model
log_fit2 <- Boston_trn %>% 
            train(crim01 ~ nox + rad, method = "glm", family = binomial, data = .)
summary(log_fit2)  
```
```{r}
# running prediction of the simpler mode on the test data and checking results
log_pred2 <- knn_pred <- predict(log_fit2, newdata = Boston_tst, type = "raw")
log_tab2 <- table(predicted = log_pred2, actual = Boston_tst$crim01)
log_conf2 <- confusionMatrix(log_tab2, positive = "1")
log_conf2
```
Lost about 6% accuracy

##### 13b LDA on Boston Data
```{r}
# using same training/test data. Starting w/ all the predictors except crim
lda_fit <- Boston_trn %>% 
           train(crim01 ~ . -crim, method = "lda", data = .)
lda_fit$finalModel # for LDA the summary is in $final$Model
```
```{r}
lda_pred <- predict(lda_fit, newdata = Boston_tst, type = "raw") # getting the classification results rather than the probs
lda_tab <- table(predicted = lda_pred, actual = Boston_tst$crim01)
lda_conf <- confusionMatrix(lda_tab, positive = "1")
lda_conf
```
86 % accuracy w/ all the predictors. Logistic was almost 92% accurate

##### 13c - QDA on the Boston Data

```{r}
qda_fit <- Boston_trn %>% 
           train(crim01 ~ . -crim, method = "qda", data = .)
qda_fit$finalModel # for QDA the summary is in $final$Model
```
```{r}
qda_pred <- predict(qda_fit, newdata = Boston_tst, type = "raw") # getting the classification results rather than the probs
qda_tab <- table(predicted = qda_pred, actual = Boston_tst$crim01)
qda_conf <- confusionMatrix(qda_tab, positive = "1")
qda_conf
```
QDA is a little better than LDA

##### 13d - kNN on the Boston Data Set
```{r}
# for kNN - need to scale all predictors since they have measurement scales whic will effect distance mesurements in kNN
Boston_trn <- Boston_trn %>% 
            mutate_if(is.numeric, funs(scale(.)))
Boston_tst <- Boston_tst %>% 
            mutate_if(is.numeric, funs(scale(.)))
```
```{r}
# letting train find the best kNN - using all predictors except crim 
ctlr <- trainControl(method = "cv", number = 10) # using cross-validtion w/ 10 folds
knn_fit <- Boston_trn %>% 
                    train(crim01 ~ . - crim , data = ., method = "knn", trControl = ctlr, preProcess = c("center", "scale"), tuneLength = 50, na.action = "na.omit", metric = "Kappa")
print(knn_fit)
```
```{r}
plot(knn_fit)
```
```{r}
knn_pred <- predict(knn_fit, newdata = Boston_tst, type = "raw")
knn_tab <- table(predicted = knn_pred, actual = Boston_tst$crim01)
knn_conf <- confusionMatrix(knn_tab, positive = "1")
knn_conf
```
kNN w/ k = 5 is almost 90% accurate. So, logistic and kNN are better than LDA and QDA on this data set.
