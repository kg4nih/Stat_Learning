...
ctlr <- trainControl(method = "repeatedcv", number = 10, repeats = 3) # using cross-validtion w/ 10 folds and 3 repeats
...
```
```{r}
# taking a 20% sample for training
sample_index <- sample(nrow(Caravan_scaled), trunc(.20 * nrow(Caravan_scaled)), replace = FALSE)
head(sample_index)
head(Caravan_scaled)
...
print(knn_fit) # kNN fitting results; note training() used k nearest neighbors from 5 - 43 and then used k = 5 for the final fitting based on Kappa
```
```{r}
plot(knn_fit) # plotting Kappa vs #Neighbors
```
...
knn_pred <- predict(knn_fit, newdata = Caravan_tst)
# building a pred vs actual table to feed to confusionMatrix()
knn_pred_tab <- table(actual = Caravan_tst$Purchase, predicted = knn_pred)
