...
#### 4.6.1 Loading and looking at the Stock Market data
```{r}
# I'm going to try and use tidyverve and caret to do this lab
Smarket <- as.tibble(Smarket)
summary(Smarket)
```
```{r}
# plotting Smarket using ggpairs()
ggpairs(Smarket, progress = FALSE)
```
```{r}
# plotting volume by year
Smarket %>% 
  ggplot(aes(x = Year, y = Volume)) +
  geom_point() + 
  geom_smooth(method = lm)
```
Plot shows that on average volume has been increasing from 2001 - 2005
```{r}
# splitting data into training and test sets and fitting a logistic regression model using glm
Smarket_trn <- Smarket %>% 
              filter(Year < 2005)
Smarket_tst <- Smarket %>% 
                filter(Year >= 2005)
Smarket_log_fit <- Smarket_trn %>% 
                    glm(Direction ~. - Year - Today, data = ., family = binomial)
```
```{r}
...
Smarket_log_prob <- predict(Smarket_log_fit, newdata = Smarket_tst, type = "response")
Smarket_log_prob[1:10]
# converting probs to Direction
Smarket_log_pred <- ifelse(Smarket_log_prob > 0.5, "Up", "Down")
# building a pred vs actual table to feed to confusionMatrix()
Smarket_log_pred_tab <- table(actual = Smarket_tst$Direction, predicted = Smarket_log_pred)
Smarket_log_conf <- confusionMatrix(Smarket_log_pred_tab, positive = "Up")
Smarket_log_conf

```
```{r}
calc_class_err(actual = Smarket_tst$Direction, predicted =  Smarket_log_pred)
```
```{r}
# redoing the log model w/ Lag1 & Lag2 as predictors
Smarket_log_fit_2 <- Smarket_trn %>% 
                      glm(Direction ~ Lag1 + Lag2, data =.,family = binomial)
summary(Smarket_log_fit_2)
```
```{r}
Smarket_log_prob_2 <- predict(Smarket_log_fit_2, newdata = Smarket_tst, type = "response")
Smarket_log_pred_2 <- if_else(Smarket_log_prob_2 > 0.5, "Up", "Down")
Smarket_log_pred_tab_2 <- table(actual = Smarket_tst$Direction, predicted = Smarket_log_pred_2)
Smarket_log_conf_2 <- confusionMatrix(Smarket_log_pred_tab_2, positive = "Up")
Smarket_log_conf_2
```
```{r}
calc_class_err(actual = Smarket_tst$Direction, predicted = Smarket_log_pred_2)
```
...
Default_knn_trn <- Default_knn_trn %>% 
                   select(-index)
head(Default_knn_trn)
