...
Smarket_tst <- Smarket %>% 
                filter(Year >= 2005)
Smarket_fit <- Smarket_trn %>%  
                train(Direction ~ Lag1 + Lag2, method = "glm", family = binomial, data = .)
Smarket_fit
```
```{r}
Smarket_prob <- predict(Smarket_fit, newdata = Smarket_tst, type = "prob")
Smarket_prob
```
```{r}
Smarket_pred <- Smarket_prob %>% 
                mutate(Direction = if_else(Up > 0.5, "Up", "Down"))
Smarket_pred
```
```{r}
Smarket_tab <- table(predicted = Smarket_pred$Direction,actual = Smarket_tst$Direction)
Smarket_tab
confusionMatrix(Smarket_tab, positive = "Up")
```
```{r}
...
```
```{r}
calc_class_err(actual = Smarket_tst$Direction, predicted = Smarket_pred$Direction)
```
